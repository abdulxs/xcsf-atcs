...
def preprocess_xcs(state, scenario_metadata, histlen):
    ...
    history = [True if res else False for res in state['LastResults'][0:histlen]]

    if len(history) < histlen:
        history.extend([True] * (histlen - len(history)))

    situation = {}
    duration = state['Duration'] / scenario_metadata['totalTime']
    situation["duration"] = duration
    situation["previous_results"] = history
    situation["last_execution"] = time_since
    assert 0 <= duration and duration <= 2
    assert 0 <= time_since and time_since <= 1.00000001
    return situation

def preprocess_xcs_discrete(state, scenario_metadata, histlen):
    ...
    situation["duration"] = duration
    situation["previous_results"] = history
    situation["last_execution"] = time_group
    return situation
